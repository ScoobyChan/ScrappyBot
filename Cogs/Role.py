...
class Role(commands.Cog):
	...
	@commands.command()
	async def defaultrole(self, ctx, roleName=None):
		r = self.settings.Get(ctx, 'role', roleName)
		if not r:
			return await ctx.send("Role doesn't exist")

		msg = await ctx.send('Removing Old Role')

		bef = self.settings.Get(ctx, 'role', self.settings.ServerConfig(ctx.guild.id, 'DefaultRole'))
		if bef:
			for m in ctx.guild.members:
				if bef in m.roles:
					await m.remove_roles(bef)
		...
		xprole = self.settings.ServerConfig(ctx.guild.id, 'XPRoles')
		if not r.id in [_role[0] for _role in xprole]:
			xprole.append((r.id, 0))
		else:
			for _role in xprole:
				if _role[0] == r.id:
					xprole.remove(_role)
			...
			xprole.append((r.id, 0))
		self.settings.ServerConfig(ctx.guild.id, 'XPRoles', xprole)

		await msg.edit(content=f'Setting Default Role to {r.mention}')
