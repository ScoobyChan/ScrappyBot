...
class ChannelLimit(commands.Cog):
	...
	async def onmessage(self, message):
		...
		# set this up for multiple channels than single 

		channels = SettingJson.get('Server', {}).get(str(message.guild.id), {}).get('ChannelContent', {}).get(str(message.channel.id), None)
		if channels == None:
			return
	...
	def ResetChannel(self, ctx, channel: discord.TextChannel=None):
		SettingJson = self.settings.LoadSettings()
	...
	@commands.command()
	async def setChannelLimitSettings(self, ctx, channel: discord.TextChannel=None):
	...
	@commands.command()
	async def checkChannelLimit(self, ctx, channel: discord.TextChannel=None):
		"""
