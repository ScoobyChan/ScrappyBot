...
class Quote(commands.Cog):
	...
	async def sendQuote(self, gID, chID, uID, msgID, ctx=None, quote:str=None):
		if self.settings.Get(Type='g', Name=gID):
			guild = self.settings.Get(Type='g', Name=gID)
		else:
			return
		
		ch = self.bot.get_channel(chID)
		quotech = self.bot.get_channel(self.settings.ServerConfig(gID, 'quoteChannel'))

		u = find(lambda g : g.id == uID, guild.members)		
		if not quotech:
			return
